...
def default_password(user_id: str, role: str) -> str:
    ...
    if role in ("admin", "hod"):
        return f"Sahay@{role.capitalize()}2025"
    # section mentors: Sahay@Mentor + last 4 chars of their id
    return f"Sahay@{user_id[-4:].upper()}2025"
...
def seed_users(con, mentors: dict):
    ...
    for uid, info in mentors.items():
        ...
        con.execute(
            "INSERT INTO users (id,name,role,dept,section,password_hash,salt,created_at)"
            " VALUES (?,?,?,?,?,?,?,?)",
            (uid, info["name"], info["role"],
...
def login(con, user_id: str, password: str):
    ...
    ensure_auth_schema(con)
    row = con.execute(
        "SELECT id,name,role,dept,section,password_hash,salt FROM users WHERE id=?",
        (user_id.lower().strip(),)
    ).fetchone()
    if not row:
...
def verify_token(con, token: str):
    ...
    ensure_auth_schema(con)
    row = con.execute(
        "SELECT s.user_id, s.expires_at, u.name, u.role, u.dept, u.section "
